# 20250924 - Versione modificata per gestire più volti per immagine - Windows Compatible
...
class MultiFaceRecognitionApp:
    def __init__(self):
        ...
        # Cerca il file shape predictor in diverse posizioni tipiche di Windows
        predictor_paths = [
            "shape_predictor_68_face_landmarks.dat",
            os.path.join(os.getcwd(), "shape_predictor_68_face_landmarks.dat"),
            os.path.join(self.work_dir, "shape_predictor_68_face_landmarks.dat"),
        ...
        if shape_predictor_path:
            ...
            print("ATTENZIONE: File shape_predictor_68_face_landmarks.dat non trovato!")
            print("Scaricare da: http://dlib.net/files/shape_predictor_68_face_landmarks.dat.bz2")
            self.predictor = None
    ...
    def image_parsing(self):
        """Converte immagini in formato JPG usando PIL (compatibile Windows)"""
        ...
        for directory_name in ['01.known_sources', '02.unknown_targets']:
            ...
            if os.path.exists(directory):
                for ext in extensions:
                    ...
                    if files:
                        print(f"Convertendo {len(files)} file {ext.upper()} in JPG...")
                        # Converti in JPG usando PIL
                        for file_path in files:
                            try:
                                # Apri con PIL e salva come JPG
                                with Image.open(file_path) as img:
                                    ...
                                    print(f"Convertito: {os.path.basename(file_path)} -> {base_name}.jpg")
                                
                                # Sposta il file originale in TMP
                                tmp_path = os.path.join(self.tmp_dir, os.path.basename(file_path))
    ...
    def draw_border(self, frame, pt1, pt2, color, thickness, r, d):
        """Disegna un bordo arrotondato intorno al volto"""
        x1, y1 = pt1
        x2, y2 = pt2
    ...
    def recon_landmarks(self, source_dir, destination_dir):
        ...
        for file in files:
            try:
                ...
                # Elabora landmarks solo se dlib predictor è disponibile
                if self.predictor:
                ...
                for top, right, bottom, left in face_locations:
                    # Scala le coordinate dal frame ridotto a quello originale
                    top *= 4
                    right *= 4
                    bottom *= 4
                    left *= 4
    ...
    def multi_face_recognition(self, tolerance_percent):
        ...
        if not os.path.exists(self.known_sources):
            print(f"Directory {self.known_sources} non trovata!")
            return False
        ...
        if not os.path.exists(self.unknown_targets):
            print(f"Directory {self.unknown_targets} non trovata!")
            return False
    ...
    def generate_multiface_report(self, tolerance_percent):
        """Genera il report HTML per volti multipli - versione Windows compatible"""
        ...
        # HTML senza emoji per compatibilità Windows
        html_content = '''<!DOCTYPE html>
<html><head>
<meta charset="UTF-8">
<title>Multi-Face Recognition Results</title>
<style>
...
'''
        ...
        for i, result in enumerate(results, 1):
            ...
            # Determina la qualità del match
...
<div class="match-container" 
    ...
    <div class="image-pair">
        ...
        <div class="match-info">
            ...
            <span class="{quality_class}">{quality_symbol} {quality_text}</span></p>
...
<div style="text-align: center; margin: 20px; color: #666;">
    ...
    <p><span class="quality-poor">[*] &lt;40% Insufficient</span> | 
       <span class="quality-fair">[**] &gt;40% Minimum</span> | 
       <span class="quality-good">[***] &gt;50% Optimal</span> | 
       <span class="quality-excellent">[****] &gt;60% Maximum</span></p>
    <p><em>Face landmarks are shown only for matched faces</em></p>
    <p><tt>Enhanced by Visi@n - Multi-Face Recognition System</tt></p>
</div>
</body>
</html>'''
        
        # Scrivi il file HTML con encoding UTF-8
        try:
            with open(index_file, 'w', encoding='utf-8') as f:
                f.write(html_content)
            print(f"Report HTML generato con successo: {index_file}")
            return index_file
        except Exception as e:
    ...
    def show_multiface_report(self):
        ...
        if os.path.exists(index_file):
            try:
                webbrowser.open(f'file:///{index_file.replace(os.sep, "/")}')
                print(f"Apertura browser con: {index_file}")
            except Exception as e:
                print(f"Errore nell'apertura del browser: {e}")
                print(f"Aprire manualmente: {index_file}")
    ...
    def process_multiface_recognition(self, tolerance_percent):
        ...
        if self.multi_face_recognition(tolerance_percent):
            report_file = self.generate_multiface_report(tolerance_percent)
            if report_file:
...
def openWorkingDirectory():
    ...
    for directory in directories:
        full_path = os.path.join(home_dir, directory.replace('/', os.sep))
        if not os.path.exists(full_path):
            os.makedirs(full_path)
            print(f"Creata directory: {full_path}")
    
    # Apri il file manager (Windows Explorer)
    try:
        os.startfile(work_dir)
    except AttributeError:
        # Fallback per sistemi non Windows
        try:
            subprocess.run(['explorer', work_dir], shell=True)
        except:
            subprocess.run(['xdg-open', work_dir])  # Linux fallback
...
# GUI principale aggiornata per Windows
if __name__ == "__main__":
    root = tk.Tk()
    frame = tk.Frame(root)
    frame.pack()
    root.wm_title("Multi-Face Recognition - Windows")
    ...
    # Verifica dipendenze
    deps_label = tk.Label(text="Dependencies check...",
                         fg="orange",
                         font=("helvetica", 8))
    deps_label.pack()
    deps_label.place(x=20, y=75)

    # Verifica shape predictor
    shape_predictor_found = False
    predictor_paths = [
        "shape_predictor_68_face_landmarks.dat",
        os.path.join(os.getcwd(), "shape_predictor_68_face_landmarks.dat")
    ]
    ...
    for path in predictor_paths:
        if os.path.exists(path):
            shape_predictor_found = True
            break
    
    if shape_predictor_found:
        deps_label.config(text="Dependencies: OK", fg="green")
    else:
        deps_label.config(text="Missing: shape_predictor_68_face_landmarks.dat", fg="red")
